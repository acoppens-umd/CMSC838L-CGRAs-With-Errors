...
namespace HyCUBESim {
	...
	void CGRATile::execute(int kII) {
		...
		switch(currIns.opcode){
			...
			//ALEX
			case ADD_WO:
				LOG(SIMULATOR) << ": ADD_WO," << operand1 << "," << operand2 << "\n";
				{
			...
			case MUL_WO:
				...
				bool overflow = prod > std::numeric_limits<int32_t>::max() ||
...
//				ALUTempOut = I1isvalid ? operand1 : operand2;
			...
			case LOADB :
				...
				break;
			case LOADD :
				LOG(SIMULATOR) << ": LOADD," << operand1 << "," << operand2 << "\n";
				ALUTempOut = load(operand2,8);
				break;
			case STORE :
			...
			case STOREB :
				...
				break;
			case STORED :
				LOG(SIMULATOR) << ": STORED," << operand1 << "," << operand2 << "\n";
				if(!Pisvalid || predicate){
					//only store after checking predicate
					//other operations dont care as the output is not routed.
					ALUTempOut = store(operand1,operand2,8);
				}
				break;
			case JUMPL :
...
//		assert(sinkedtoReg);
	...
	DataType CGRATile::load(DataType op2, int size) {
		...
		if(size == 1){
			...
			//assert(op2 % 8 == 0);
			res = res | static_cast<DataType>((*dmemPtr)[op2]) | (static_cast<DataType>((*dmemPtr)[op2+1]) << 8) | 
						(static_cast<DataType>((*dmemPtr)[op2+2]) << 16) | (static_cast<DataType>((*dmemPtr)[op2+3]) << 24) |
					    (static_cast<DataType>((*dmemPtr)[op2+4]) << 32) | (static_cast<DataType>((*dmemPtr)[op2+5]) << 40) | 
						(static_cast<DataType>((*dmemPtr)[op2+6]) << 48) | (static_cast<DataType>((*dmemPtr)[op2+7]) << 56);
		} 
	...
	DataType CGRATile::store(DataType op1, DataType op2, int size) {
		...
		if(size == 1){
			...
			//assert(op2 % 4 == 0);
			uint8_t stdata0 = op1 & 0x00000000000000ff;
			uint8_t stdata1 = (op1 & 0x000000000000ff00) >> 8;
			uint8_t stdata2 = (op1 & 0x0000000000ff0000) >> 16;
			uint8_t stdata3 = (op1 & 0x00000000ff000000) >> 24;
			uint8_t stdata4 = (op1 & 0x000000ff00000000) >> 32;
			uint8_t stdata5 = (op1 & 0x0000ff0000000000) >> 40;
			uint8_t stdata6 = (op1 & 0x00ff000000000000) >> 48;
			uint8_t stdata7 = (op1 & 0xff00000000000000) >> 56;

			(*dmemPtr)[op2] = stdata0;
			(*dmemPtr)[op2+1] = stdata1;
			(*dmemPtr)[op2+2] = stdata2;
			(*dmemPtr)[op2+3] = stdata3;
			(*dmemPtr)[op2+4] = stdata4;
			(*dmemPtr)[op2+5] = stdata5;
			(*dmemPtr)[op2+6] = stdata6;
			(*dmemPtr)[op2+7] = stdata7;
		}
		return 1;
	}
