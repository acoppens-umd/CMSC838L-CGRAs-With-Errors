...
//			uint32_t arrayAddrPtr=0;
			...
			std::set<std::pair<BasicBlock*,BasicBlock*>> looppanicBB;

			std::map<int, std::string> exitMessages;
			int loopIdx=-1;
...
//			void InsertNode(dfgNode Node);
			...
			//ALEX
			int getExitCode(BasicBlock* in, BasicBlock* out);
...
//			int findUtilTreeRoutingLocs(CGRANode* cnode, dfgNode* currNode);
			...
			void setLoopBB(std::set<BasicBlock*> BBs,std::set<std::pair<BasicBlock*,BasicBlock*>> entryBBs, 
							std::set<std::pair<BasicBlock*,BasicBlock*>> exitBBs,std::set<std::pair<BasicBlock*,BasicBlock*>> panicBBs){loopBB=BBs;loopentryBB=entryBBs;
																																	    loopexitBB=exitBBs;looppanicBB=panicBBs;}
