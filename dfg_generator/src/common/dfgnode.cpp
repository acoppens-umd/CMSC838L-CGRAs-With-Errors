...
dfgNode* dfgNode::addCMergeParent(dfgNode* phiBRAncestor,dfgNode* phiDataAncestor,int64_t constVal,bool selectOp) {
...
dfgNode* dfgNode::addCMergeParent(Instruction* phiBRAncestorIns, int64_t constVal) {
	dfgNode* temp = new dfgNode(Parent);
	temp->setIdx(Parent->getNodesPtr()->size());
	Parent->getNodesPtr()->push_back(temp);
	temp->setNameType("CMERGE");
