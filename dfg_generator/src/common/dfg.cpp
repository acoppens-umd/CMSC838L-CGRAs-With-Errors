...
//ALEX
int DFG::getExitCode(BasicBlock* in, BasicBlock* out) {
	int i = 0;
	for (const std::pair<BasicBlock*,BasicBlock*>& panicExit : this->looppanicBB) {
        if (panicExit.first == in && panicExit.second == out) {
			return i + 2;
		}
...
void DFG::printExitCodes() {
	std::string fileName = kernelname + "_panic_codes.txt";

	std::ofstream txtFile;
	txtFile.open(fileName.c_str());

	txtFile << "exit_code,message\n";

	for (const std::pair<int, std::string> & pair : exitMessages) {
		txtFile << pair.first << "," << pair.second << "\n";
	}
...
int DFG::removeNode(dfgNode *n)
{
	...
	LLVM_DEBUG(dbgs() << "Printing Parents of Removed Node \n");
	//Treat the ancestors
	...
	LLVM_DEBUG(dbgs() << "Printing Children of Removed Node \n");
	//Treat the children
...
//Alex
void DFG::removeUnusedNodes()
{
	LLVM_DEBUG(dbgs() << "Removing Unused Nodes\n");
	int i = 0;
	while (i < NodeList.size()) 
	{
		dfgNode * nodePtr = NodeList[i];
		Instruction* nodeInst = nodePtr->getNode();

		bool removed = false;
		
		if (nodeInst != NULL)
		{
			switch (nodeInst->getOpcode()) 
			{
				case Instruction::Call:
				{
					CallInst *call = llvm::dyn_cast<CallInst>(nodeInst);
					Function *fun = call->getCalledFunction();

					std::string functionName = fun->getName().str();
					LLVM_DEBUG(dbgs() << "Found Function Call to: " << fun->getName().str() << "\n");

					if (functionName == "llvm.assume") {
						removed = this->removeNode(nodePtr) == 1;
					}
				}
				break;
				default:
...
DFG::DFG(std::string name, std::map<Loop *, std::string> *lnPtr)
{
	...
	HyCUBEInsStrings[Hy_P_LOADD] = "LOADD";
	HyCUBEInsBinary[Hy_P_LOADD] = 0 | (0b100000);
	...
	HyCUBEInsStrings[Hy_P_STORED] = "STORED";
	HyCUBEInsBinary[Hy_P_STORED] = 0 | (0b100001);
	...
	for (int i = 0; i < NodeList.size(); ++i)
	{
		...
		if (node->getNode() != NULL)
		{
			...
			case Instruction::Trunc:
			{
				...
				uint64_t bitMask = ((uint64_t)bitMaskDbl);
			...
			case Instruction::SExt:
			{
				...
				uint64_t srcByteWidth = SI->getSrcTy()->getIntegerBitWidth() / 8;
				uint64_t destByteWidth = SI->getDestTy()->getIntegerBitWidth() / 8;
				uint64_t constOperand = (srcByteWidth << 16) | destByteWidth;
			...
			case Instruction::ExtractValue:
				//Unfortunately, we're squishing all primitives to 32bit, so we only support 32 | 32 aggregates
				{
				llvm::ExtractValueInst * ev_inst = llvm::dyn_cast<llvm::ExtractValueInst>(node->getNode());

				llvm::Type *AggType = ev_inst->getAggregateOperand()->getType();
				llvm::Type *FirstElemType = nullptr;

				if (auto *ST = llvm::dyn_cast<llvm::StructType>(AggType)) {
					// { T0, T1, ... }
					assert(ST->getNumElements() != 0);
					FirstElemType = ST->getElementType(0);

				} else if (auto *AT = llvm::dyn_cast<llvm::ArrayType>(AggType)) {
					// [N x T]
					FirstElemType = AT->getElementType();

				} else if (auto *VT = llvm::dyn_cast<llvm::VectorType>(AggType)) {
					// <N x T>
					FirstElemType = VT->getElementType();

				} else {
					// Not an aggregate type — extractvalue would be invalid here
					assert(false);
				}

				double destBitWidthDbl = (double)(FirstElemType->getIntegerBitWidth());

				if (ev_inst->getIndices()[0] == 0) {
					double bitMaskDbl = pow(2, destBitWidthDbl) - 1;
					uint64_t bitMask = ((uint64_t)bitMaskDbl);
					node->setConstantVal(bitMask);
					node->setFinalIns(AND);
				} else if (ev_inst->getIndices()[0] == 1) {
					node->setConstantVal(destBitWidthDbl);
					node->setFinalIns(RS);
				} else {
					assert(false && "Not supporting extract value for index > 1");
				}
				}
				break;
			case Instruction::Call:
				{
				CallInst *call = llvm::dyn_cast<CallInst>(node->getNode());
				Function *fun = call->getCalledFunction();

				std::string functionName = fun->getName().str();
				LLVM_DEBUG(dbgs() << "While Naming Nodes Found Function Call to: " << fun->getName().str() << "\n");

				if (functionName == "llvm.smul.with.overflow.i32") {
					LLVM_DEBUG(dbgs() << "Adding Multiply with Overflow \n");
					node->setFinalIns(MUL_WO);
					break;
		...
		else
		{
			...
			else if (node->getNameType().compare("OutLoopSTORE") == 0)
			{
				...
				else
				{
					...
					{
						Instruction* inst = dyn_cast<Instruction>(OutLoopNodeMapReverse[node]);
						this->dataLayout = inst->getParent()->getParent()->getParent()->getDataLayout();
						uint64_t sizeInBits = this->dataLayout->getTypeAllocSizeInBits(OutLoopNodeMapReverse[node]->getType());
			...
			else if (node->getNameType().compare("OutLoopLOAD") == 0)
			{
				...
				else
				{
					...
					else if (OutLoopNodeMapReverse[node]->getType()->isAggregateType())
					{
						//TODO : second look
						node->setFinalIns(Hy_LOAD);
					}
					else
					{
						...
						OutLoopNodeMapReverse[node]->getType()->print(rso);
						LLVM_DEBUG(dbgs() << "OutLoopLoadType = " << rso.str());
						assert(0);
					}
				}
				node->setConstantVal(node->getoutloopAddr());
		...
		if (PHINode *phiIns = dyn_cast<PHINode>(node->getNode()))
		{
			...
			for (int k = 0; k < phiIns->getNumIncomingValues(); ++k)
			{
				...
				else if (UndefValue *UND = dyn_cast<UndefValue>(phiIns->getIncomingValueForBlock(bb)))
				{
					...
					if (LoopBB.find(brIns->getParent()) == LoopBB.end())
					{
						node->addCMergeParent(brIns, INT64_C(0));
					}
					else
					{
						assert(brNode != NULL);
...
void DFG::GEPInvestigate(Function &F, Loop *L, std::map<std::string, int> *sizeArrMap)
{
	...
	for (int i = 0; i < NodeList.size(); ++i)
	{
		...
		{
			this->dataLayout = ins->getParent()->getParent()->getParent()->getDataLayout();
			...
			if (dyn_cast<StructType>(T))
			{
				...
				int size = this->dataLayout->getTypeAllocSize(ST);
			...
			else
			{
				if (dyn_cast<ArrayType>(T))
				{
					...
					LLVM_DEBUG(dbgs() << "Size = " << this->dataLayout->getTypeAllocSize(AT) << "\n");
					...
					Value *old = GEP->getPointerOperand();
					Value *bitcastedPtr = builder.CreateBitCast(old, Type::getInt8PtrTy(Ctx));
					int size = this->dataLayout->getTypeAllocSize(AT);
				...
				else
				{
					...
					Value *addr = GEP;
					Value *bitcastedPtr = builder.CreateBitCast(old, Type::getInt8PtrTy(Ctx));
					int size = this->dataLayout->getTypeAllocSize(IT);
...
int DFG::findOperandNumber(dfgNode *node, Instruction *child, Value *parent)
{
	...
	{
		int num_ops = child->getNumOperands();
		if (CallInst* CI = dyn_cast<CallInst>(child)) {
			num_ops--;
		}
...
{
	LLVM_DEBUG(dbgs() << "handlestartstop_munit!\n");
...
int DFG::nonGEPLoadStorecheck()
{
	...
	for (int i = 0; i < NodeList.size(); ++i)
	{
		...
		this->dataLayout = node->getNode()->getParent()->getParent()->getParent()->getDataLayout();
		...
		LLVM_DEBUG(node->getNode()->dump());
		int Size = this->dataLayout->getTypeAllocSize(T);
...
void DFG::printREGIMapfiles()
{
	...
	regimapInsIdxMap[Hy_P_STORED] = 21;
...
int DFG::CalculateGEPBaseAddr(GetElementPtrInst *GEP)
{
	...
	int base_addr = 0;
	this->dataLayout = GEP->getParent()->getParent()->getParent()->getDataLayout();
	...
	if (StructType *ST = dyn_cast<StructType>(GEP->getSourceElementType()))
	{
    	LLVM_DEBUG(dbgs() << "\t");
    	LLVM_DEBUG(ST->dump());
    	total_size = this->dataLayout->getTypeAllocSize(ST);
    	// LLVM_DEBUG(dbgs() << "\t\t sample element 2 = " ; ST->getElementType(2)->dump();
	}
	else if (ArrayType *AT = dyn_cast<ArrayType>(GEP->getSourceElementType()))
	{
   	 LLVM_DEBUG(dbgs() << "\t");
    	LLVM_DEBUG(AT->dump());
    	total_size = this->dataLayout->getTypeAllocSize(AT);
	}
	else
	{
			...
			for (int i = 2; i < GEP->getNumOperands(); i++)
			{
				...
				if (ConstantInt *CV = dyn_cast<ConstantInt>(GEP->getOperand(i)))
				{
					...
					if (StructType *ST = dyn_cast<StructType>(currType))
					{
						...
						populateStructOffset(ST, structOffset, *(this->dataLayout));
					...
					else if (ArrayType *AT = dyn_cast<ArrayType>(currType))
					{
						...
						int element_size = this->dataLayout->getTypeAllocSize(AT->getElementType());
		...
		std::unordered_map<Value *, GetElementPtrInst *> &mem_ptrs, std::map<dfgNode*,Value*> &OLNodesWithPtrTyUsage, Function &F)
{
	...
	for (dfgNode *node : NodeList)
	{
		...
		if (OutLoopNodeMapReverse.find(node) != OutLoopNodeMapReverse.end())
		{
			...
			this->dataLayout = F.getParent()->getDataLayout();
			array_pointer_sizes[OutLoopNodeMapReverse[node]->getName().str()] = this->dataLayout->getTypeAllocSize(OutLoopNodeMapReverse[node]->getType());
		...
		{
			LLVM_DEBUG(dbgs() << "----------May read or write:");
			LLVM_DEBUG(node->getNode()->dump());
			if (LoadInst *LDI = dyn_cast<LoadInst>(node->getNode()))
			{
				...
				if(sizeArrMap.find(pointer->getNameOrAsOperand()) != sizeArrMap.end()){
					...
					else
					{
					    ...
					    this->dataLayout = LDI->getParent()->getParent()->getParent()->getDataLayout();
						PointerType *PT = cast<PointerType>(mem_ptrs[pointer]->getPointerOperand()->getType());
						if (!PT->isOpaque())
			...
			else if (StoreInst *STI = dyn_cast<StoreInst>(node->getNode()))
			{
				...
				if(sizeArrMap.find(pointer->getNameOrAsOperand()) != sizeArrMap.end()){
					...
					else
					{
						...
						// LLVM_DEBUG(dbgs() << "Fix this: base_ptr_name:" << base_ptr_name <<  "array pointer size:" << array_pointer_sizes[base_ptr_name]<< "\n");
					    this->dataLayout = STI->getParent()->getParent()->getParent()->getDataLayout();
						PointerType *PT = cast<PointerType>(mem_ptrs[pointer]->getPointerOperand()->getType());
						if (!PT->isOpaque())
							array_pointer_sizes[base_ptr_name] = this->dataLayout->getTypeAllocSize(PT->getPointerElementType());
						else 
		...
		else if (node->getNode() && isa<GetElementPtrInst>(node->getNode()))
		{
			...
			else
			{
				...
				this->dataLayout = GEP->getParent()->getParent()->getParent()->getDataLayout();
				PointerType *PT = cast<PointerType>(GEP->getPointerOperand()->getType());
				if (array_pointer_sizes.find(base_ptr_name) == array_pointer_sizes.end()) {
					if (!PT->isOpaque())
							array_pointer_sizes[base_ptr_name] = this->dataLayout->getTypeAllocSize(PT->getPointerElementType());
						else 
							array_pointer_sizes[base_ptr_name] = 4;
...
void DFG::InstrumentInOutVars(Function &F, std::unordered_map<Value *, int> mem_accesses, std::map<dfgNode*,Value*> &OLNodesWithPtrTyUsage, std::unordered_map<Value *, int> &spm_base_address){
	...
	//ALEX
	auto live_out_report_carry = F.getParent()->getOrInsertFunction(
		"LiveOutReportCarry",
		FunctionType::getVoidTy(Ctx),
		Type::getInt8PtrTy(Ctx), //variable name
		Type::getInt8Ty(Ctx) //carry data
	);
	...
	for(auto it = mem_accesses.begin(); it != mem_accesses.end(); it++){
			...
			for (auto i : OLNodesWithPtrTyUsage)
			{
				...
				if(ptrs == ptr){
					...
					if (GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(ptrs)) {
						...
						break;
					} else if (IntToPtrInst *ITP = dyn_cast<IntToPtrInst>(ptrs)) {  
						LLVM_DEBUG(dbgs() << "IntToPtr instruction detected\n");  
						LLVM_DEBUG(ITP->dump());  
						
						Value* bitcastedptr = builder.CreateBitCast(ITP, Type::getInt8PtrTy(Ctx));  
		...
		for(auto trans : loopexitBB){
			...
			LLVM_DEBUG(dbgs() << "EXIT_BB:\n");
			LLVM_DEBUG(exitBB->dump());
						...
						for (auto &I : *exitBB){
							...
							if(I.getOperand(0)==ptr){
								...
								if (valueType == Type::getInt64Ty(Ctx)) {
									...
									Value* carryval = builder.CreateExtractValue(val, 1);
									Value* castedcarryval = builder.CreateZExt(carryval, Type::getInt8Ty(Ctx));
									builder.CreateCall(live_out_report_carry,{ptr_name_val,castedcarryval});
								} else {
...
//						builder.CreateCall(live_out_report_intvar_FN,{ptr_name_val,bitcastedptr});
	...
	for(auto trans : loopexitBB){
		BasicBlock* exitBB = trans.second;	
		UnreachableInst *unreachableTerminator = dyn_cast<UnreachableInst>(exitBB->getTerminator());
		...
		if (unreachableTerminator) {
			...
			for (Instruction &inst : *exitBB) {
				...
				if (CallInst *CI = dyn_cast<CallInst>(instPtr)) {
					Function *Callee = CI->getCalledFunction();
					if (!Callee) return;

					StringRef Name = Callee->getName();
					if (Name.contains("panicking") && Name.contains("panic")) {
						if (found_panic) {
							assert(false && "Multiple Panics Found");
						}
						found_panic = true;
			...
			if (!found_panic) {
				assert(false && "No Panic Call Found");
			}
		} else {
			//Normal Exit
...
Value* DFG::findGlobalUse(Function *Callee) {
	for (BasicBlock &BB : *Callee) {
		for (Instruction &I2 : BB) {
			if (GlobalVariable *GV = dyn_cast<GlobalVariable>(I2.getOperand(0))) {
				return GV;
			}
		}
	}
	assert(false && "Failed to Find Global");
}

Value * DFG::stripPointerCasts(Value *V) {
    while (true) {
...
Optional<uint32_t> DFG::inferFromValue(Value *V,
    ...
    if (auto *GEP = dyn_cast<GetElementPtrInst>(V)) {
        return inferFromValue(GEP->getPointerOperand(), DL, Depth + 1, MaxDepth);
    }

    // Case 3: function argument — analyze call sites
...
int DFG::insertshiftGEPsCorrect(){
	...
	for(dfgNode* node : NodeList){
		if(node->getNode()){
			if(GetElementPtrInst* GEP = dyn_cast<GetElementPtrInst>(node->getNode())){
				this->dataLayout = GEP->getParent()->getParent()->getParent()->getDataLayout();
				int array_size = this->dataLayout->getTypeAllocSize(GEP->getSourceElementType());
