...
void loopStart(const char* loopName){
	...
	cout << ln << "------------LOOP START-------------- Iteration" << loopRunIdx[ln] <<" \n";
}
...
void loopEnd(const char* loopName){
	...
	cout << ln <<"------------LOOP END---------------- Iteration" << loopRunIdx[ln] <<" \n";
	loopRunIdx[ln]++;
}
...
void LiveOutReportCarry(const char* varname, uint8_t value){
	std::string varname_str(varname);
	//cout << "Call LiveOutReportIntermediateVar var name:" << varname_str << "Value:" <<value << "\n";
	data_morpher[varname_str + "carry"].pre_data.push_back(0);
	data_morpher[varname_str + "carry"].post_data.push_back(value);
}
