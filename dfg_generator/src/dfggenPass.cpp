...
typedef struct
{
	...
	std::set<std::pair<BasicBlock *, BasicBlock *>> panicBlocks;
} MappingUnit;
		...
		MemoryDependenceAnalysis *MD = NULL)
{
	...
	for (User *U : I->users())
	{
		...
		if (Instruction *Inst = dyn_cast<Instruction>(U))
		{
			...
			{
				LLVM_DEBUG(dbgs() << "--------------------Adding OutLoopStore for non contained use\n");
	...
	else
	{
		...
		for (std::pair<std::string, MappingUnit> pair : mappingUnitMap)
		{
			...
			for (BasicBlock *BB : mappingUnitMap[name].allBlocks)
			{
				...
				{
					BasicBlock *succBB = *it;
					Instruction* terminator = succBB->getTerminator();
					UnreachableInst* unreachable = dyn_cast<UnreachableInst>(terminator);

					if (unreachable) {
						mappingUnitMap[name].panicBlocks.insert(std::make_pair(BB, succBB));
					}
				}
			}
		}
...
{
	LLVM_DEBUG(dbgs() << "Parsing Size Attr\n");
		...
		std::map<Loop *, std::string> loopNames)
{
	...
	for (std::pair<std::string, MappingUnit> pair : mappingUnitMap)
	{
		...
		LoopDFG.setLoopBB(mappingUnitMap[munitName].allBlocks,
				...
				mappingUnitMap[munitName].panicBlocks);
...
struct dfggenPass : public FunctionPass
{
	...
	virtual bool runOnFunction(Function &F)
	{
		...
		//-----------------------------------
		{
			...
			//ALEX
			LLVM_DEBUG(dbgs() << "All blocks\n");
			for (BasicBlock* block : mappingUnitMap[munitName].allBlocks) {
				LLVM_DEBUG(dbgs() << block->getName().str() << "\n");
			}

			LLVM_DEBUG(dbgs() << "Entry blocks\n");
			for (auto entryBlock : mappingUnitMap[munitName].entryBlocks) {
				LLVM_DEBUG(dbgs() << "[First] "<< entryBlock.first->getName().str() << "\n");
				LLVM_DEBUG(dbgs() << "[Second] "<< entryBlock.second->getName().str() << "\n");
			}

			LLVM_DEBUG(dbgs() << "Exit blocks\n");
			for (auto exitBlock : mappingUnitMap[munitName].exitBlocks) {
				LLVM_DEBUG(dbgs() << "[First] " << exitBlock.first->getName().str() << "\n");
				LLVM_DEBUG(dbgs() << "[Second] " << exitBlock.second->getName().str() << "\n");
			}

			LLVM_DEBUG(dbgs() << "Panic blocks\n");
			for (auto panicBlock : mappingUnitMap[munitName].panicBlocks) {
				LLVM_DEBUG(dbgs() << "[First] " << panicBlock.first->getName().str() << "\n");
				LLVM_DEBUG(dbgs() << "[Second] " << panicBlock.second->getName().str() << "\n");
			}
