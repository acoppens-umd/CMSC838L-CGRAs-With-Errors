...
void CGRAXMLCompile::PathFinderMapper::printHyCUBEBinary(CGRA* cgra) {
	...
	LOG(PRINT_BIN) << ">>Starting Binary Printing<<\n";
				...
				for (PE *pe : peList)
				{
				   ...
				   RF0->getInPort("WP1")->getNode() == northi->getNode()){
					...
					if(mappedOP){
						...
						insF.opcode = "00000000";
					}

					if(mappedOP && mappedOP->hasConst){
						insF.constant_valid = "1";
						insF.constant = mappedOP->get32bitConstantBinaryString();
					}else if(insF.opcode == "00000000"){
						// if nop, select constant 1 to enable power gating.
					    insF.constant_valid = "1";
						insF.constant = "00000000000000000000000000000001";

					}
					else{
						insF.constant_valid = "0";
						//					insF.constant = "123456789012345678901234567";
						insF.constant = "00000000000000000000000000000000";
					}
	...
	for (int t = 0; t < this->cgra->get_t_max(); ++t)
	{
		...
		for (PE *currPE : PEList)
		{
			...
			while( temp_t <=  cgra->get_t_max()){
				if(InsFArr[getIndexOfBin(temp_t, y ,x)].opcode == "00000000"){
					nop_count++;
					temp_t++;
				}else{
					break;
			...
			tempIns.constant = std::bitset<32>(nop_count).to_string();;
		}
	}
	InsFormat jumpl;
	jumpl.negated_predicate = "0";
	jumpl.constant_valid = "1";
	jumpl.constant = "00000000000000000" + std::bitset<5>(1).to_string() + std::bitset<5>(cgra->get_t_max()).to_string() + std::bitset<5>(1).to_string();
	assert(jumpl.constant.size() == 32);
	jumpl.opcode = "00011110";
	...
	LOG(PRINT_BIN) << ">>Finished Binary Printing<<\n";
	...
	for(int t = 0 ; t < t_max ; t++){
		...
		for(int y = 0 ; y < y_max ; y++){
			for(int x = 0 ; x < x_max ; x++){
				...
				binFile << std::string(insFArr[getIndexOfBin(t, y, x)].padding, '0');
